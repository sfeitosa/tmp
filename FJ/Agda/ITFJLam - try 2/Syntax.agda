...
data Expr (Γ : Ctx) : Maybe Cl → Cl → Set where
  ...
  InvkL : ∀ {τ} i   → Expr (proj₁ (isignature (ζ Δ) i)) τ (proj₂ (isignature (ζ Δ) i))
                    → All (Expr Γ τ) (proj₁ (isignature (ζ Δ) i))
                    → Expr Γ τ (proj₂ (isignature (ζ Δ) i))
  
-- Inherently-typed values
--------------------------
  
data Val : Cl → Set where
