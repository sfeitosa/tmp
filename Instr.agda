...
data Instr (Γ : Ctx) (τ : Ty) : Ty → Set where
  ...
  ST  : τ ∈ Γ → Instr Γ τ τ
  AND : Op Γ Boolean → Instr Γ τ Boolean
  OR  : Op Γ Boolean → Instr Γ τ Boolean
  XOR : Op Γ Boolean → Instr Γ τ Boolean
  ADD : Op Γ Num → Instr Γ τ Num
  SUB : Op Γ Num → Instr Γ τ Num
  MUL : Op Γ Num → Instr Γ τ Num
  EQ  : Op Γ Num → Instr Γ τ Boolean
  GT  : Op Γ Num → Instr Γ τ Boolean
  GE  : Op Γ Num → Instr Γ τ Boolean
  NOT : Op Γ Boolean → Instr Γ τ Boolean
...
eval (suc f) e p cr (SEQ (EQ x) b) = eval f e p (eval-op e x == {!!}) b
eval (suc f) e p cr (SEQ (GT x) b) = eval f e p ({!!} < {!!}) b
eval (suc f) e p cr (SEQ (GE x) b) = eval f e p ({!!} < {!!}) b
eval (suc f) e p cr (SEQ (NOT x) b) = eval f e p (not (eval-op e x)) b
