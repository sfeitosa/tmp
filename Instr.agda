...
data Block (Γ : Ctx) : Set where
  SEQ : ∀ {τ τ′} → Instr Γ τ τ′ → Block Γ → Block Γ
  JMP : Label → Block Γ
  RET : Block Γ

Program : Ctx → Set
Program Γ = Vec (Block Γ) n
...
eval : ∀ {Γ τ} → Fuel → Env Γ → Program Γ → Const τ → Block Γ → Maybe (Env Γ)
eval zero e p cr b = nothing
eval (suc f) e p cr (SEQ (LD x) b) = eval f e p (eval-op e x) b
eval (suc f) e p cr (SEQ (ST v) b) = eval f (e [ v ]≔ {!!}) p cr b
