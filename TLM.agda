...
lstcons-⊂-inv : ∀ {t t'} → (listcons t) ⊂ (list t') → t ⊂ t'
lstcons-⊂-inv (lstcons p) = p
...
data Block (Δ : GCtx) (Γ : LCtx) : LCtx → Set where
  block-halt            : ∀ {Γ′} → Block Δ Γ Γ′
  block-seq             : ∀ {Γ′ Γ″} → Instr Δ Γ Γ′ → Block Δ Γ′ Γ″ → Block Δ Γ Γ″
  block-jump            : ∀ {l Γ₁ Γ′} → Δ [ l ]= Γ₁ → Γ ≺ Γ₁ → Block Δ Γ Γ′
...
Prog : GCtx → Set
Prog Δ = ∀ {Γ′} → Allv (λ Γ → Block Δ Γ Γ′) Δ
...
run-step : ∀ {Δ Γ Γ′} → Fuel → Prog Δ → LEnv Γ → Block Δ Γ Γ′ → Maybe (LEnv Γ′)
...
run-step (suc f) p r (block-seq (instr-branch-list {τ} {i} v l s) b) with lookup r v
... | nil-l rewrite sym ([]=⇒lookup l) = run-step f p (subsumption-env s (update-env r v nil)) (lookupv i p)
