...
postulate 
  dummy₁ : ∀ {τ₁ Γ} → (r : LEnv Γ) → (v′ : τ₁ ∈ Γ) → (τ : Ty) → LEnv (v′ ∷= τ)
  
update-env : ∀ {τ′ Γ} → (r : LEnv Γ) → (τ : Ty) → (i : τ′ ∈ Γ) → Val τ → LEnv (i ∷= τ)
update-env r τ i v with r [ i ]≔ {!v!}
... | r′ = {!!}
...
run-step p r (block-seq (instr-branch-list {τ} v l s) b) = run-step p {!!} b
...
... | (cons v₁ v₂) = run-step p (update-env r τ v′ v₁) b
...
... | (cons v₁ v₂) = run-step p (update-env r (list τ) v′ v₂) b
...
run-step p r (block-seq (instr-cons-upd {τ} v₀ v₁ v′ s) b) = run-step p (update-env r (listcons τ) v′
