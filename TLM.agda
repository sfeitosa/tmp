...
var-set-type : ∀ {τ τ′ Γ} → LEnv Γ → (i : τ′ ∈ Γ) → Val τ → LEnv (i ∷= τ)
var-set-type All.[] () v
var-set-type (x All.∷ xs) (here px₁) v = v All.∷ xs
var-set-type (x All.∷ xs) (there i) v = x All.∷ var-set-type xs i v
...
run-step p r (block-seq (instr-branch-list {τ} v l s) b) with lookup r v
... | cons-l v₁ v₂ = run-step p (var-set-type r v (cons v₁ v₂)) b
...
run-step {Δ} {Γ} p r (block-seq (instr-fetch-0-upd {τ} v v′) b) with lookup r v
... | (cons v₁ v₂) = run-step p (var-set-type r v′ v₁) b
run-step p r (block-seq (instr-fetch-1-new v) b) with lookup r v
... | (cons v₁ v₂) = run-step p (v₂ All.∷ r) b
run-step p r (block-seq (instr-fetch-1-upd {τ} v v′) b) with lookup r v
... | (cons v₁ v₂) = run-step p (var-set-type r v′ v₂) b
...
run-step p r (block-seq (instr-cons-upd {τ} v₀ v₁ v′ s) b) = run-step p (var-set-type r v′
